...
function parsePermissions(realmRoles: string[]): PermissionRow[] {
  ...
  realmRoles.forEach((perm) => {
    const match = perm.match(
      /^PERMISE-([^-]+)-(VIEW|CREATE|UPDATE|DELETE)-SUBMODULE-[^-]+-(.+)$/i,
    );
    if (!match) return;
...
const PALETTE = [
  {
    ...
    pillHover: "bg-violet-200 dark:bg-violet-900/60",
    dot: "bg-violet-500",
  },
  {
    pill: "bg-blue-100   text-blue-700   dark:bg-blue-900/40   dark:text-blue-300",
    pillHover: "bg-blue-200   dark:bg-blue-900/60",
    dot: "bg-blue-500",
  },
  {
    pill: "bg-teal-100   text-teal-700   dark:bg-teal-900/40   dark:text-teal-300",
    pillHover: "bg-teal-200   dark:bg-teal-900/60",
    dot: "bg-teal-500",
  },
  {
    pill: "bg-orange-100 text-orange-700 dark:bg-orange-900/40 dark:text-orange-300",
    pillHover: "bg-orange-200 dark:bg-orange-900/60",
    dot: "bg-orange-500",
  },
];
...
export function RolePermissionsModal({
  ...
  const toggle = (idx: number, action: ActionKey) =>
    setRows((prev) =>
      prev.map((r, i) => (i === idx ? { ...r, [action]: !r[action] } : r)),
    );

  // Group by domain
  const domainGroups: Record<
    string,
    { row: PermissionRow; globalIdx: number }[]
  > = {};
  rows.forEach((row, i) => {
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription className="mt-0.5 flex items-center gap-2">
                  ...
                  <Badge
                    variant="secondary"
                    className="font-medium py-0 text-xs"
                  >
                    {roleName.replace("ROLE-", "")}
                  </Badge>
                </DialogDescription>
              </div>
            </div>
              ...
              <thead>
                <tr>
                  ...
                  {ACTIONS.map((a) => (
                    <th
                      key={a.key}
                      className="py-2 px-3 text-center w-20 text-xs font-semibold text-muted-foreground tracking-wide"
                    >
                      {a.label}
                    </th>
                  ))}
                  ...
                  return items.map(({ row, globalIdx }, rowIdx) => (
                    ...
                    >
                      ...
                      {rowIdx === 0 ? (
                        ...
                        >
                          <div
                            className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold transition-colors duration-150 ${color.pill}`}
                          >
                            <span
                      ...
                      {ACTIONS.map((a) => (
                        <td
                          key={a.key}
                          className="px-3 py-3 border-t border-border/50 text-center"
                        >
